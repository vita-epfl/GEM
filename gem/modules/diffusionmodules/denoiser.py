...
from gem.util import append_dims, instantiate_from_config
...
class IDDenoiser(nn.Module):
    def __init__(self, num_frames: int = 25):
        super().__init__()
        self.num_frames = num_frames
    ...
    def forward(
        ...
        **kwargs
        ...
        c_noise = t * 3 - 1.5
        c_out = 2  # 0.5
        return c_out * network(x_t, c_noise, cond, cond_mask, self.num_frames, **kwargs)
...
class Denoiser2(nn.Module):
    ...
    def forward(
        ...
        self.num_frames = int(noised_input.shape[0] / 2)
        sigma = self.possibly_quantize_sigma(sigma)
        sigma_shape = sigma.shape
