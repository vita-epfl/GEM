...
from gem.util import append_dims, default
...
class LinearPredictionGuider(Guider):
    ...
    def __call__(self, x: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
        x_u, x_c = x.chunk(2)
        self.num_frames = int(x_u.shape[0])
        ...
        for k in c:
            if (
                ...
                try:
                    ...
                    print(uc[k].shape, c[k].shape)
                    breakpoint()
            else:
                assert torch.all(c[k] == uc[k])
                c_out[k] = c[k]
        return torch.cat([x] * 2), torch.cat([s] * 2), c_out, torch.cat([cond_mask] * 2)


class LinearPredictionGuiderTemp(Guider):
    def __init__(
        self,
        num_frames: int = 25,
        max_scale: float = 2.5,
