...
from gem.modules.attention import *
from gem.modules.diffusionmodules.util import AlphaBlender, linear, timestep_embedding
    ...
    def _forward(self, x, context=None, timesteps=None):
        ...
        if hasattr(self, "attn2"):
            if self.switch_temporal_ca_to_sa:
                ...
                x = (
                    self.attn2(self.norm2(x), context=context, batchify_xformers=True)
                    + x
                )
...
class SpatialVideoTransformer(SpatialTransformer):
    def __init__(
        ...
        self.high_fidelity_control = high_fidelity_control
        ...
        self.time_stack = nn.ModuleList(
            [
                VideoTransformerBlock(
                    ...
                    attn_mode=attn_mode,
                    disable_self_attn=disable_self_attn,
                    switch_temporal_ca_to_sa=switch_temporal_ca_to_sa,
    ...
    def forward(
        ...
        if self.use_spatial_context:
            assert (
                context.ndim == 3
            ), f"Dims of spatial context should be 3 but are {context.ndim}"
            ...
            if time_context.shape[0] == x.shape[0] // timesteps * h * w:
                # time context should be  (b * h * w) x t x c
                time_context = time_context
                if time_context.ndim != 3:
                    raise ValueError(
                        f"Time context should have 3 dims but has {time_context.ndim}"
                    )
            else:
        ...
        if not self.high_fidelity_control:
            fd_context = None
        ...
        for block, mix_block in zip(self.transformer_blocks, self.time_stack):
            x = block(x, context=spatial_context, fd_context=fd_context)
