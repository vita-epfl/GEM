...
class FeedForward(nn.Module):
    def __init__(
        self, dim, dim_out=None, mult=4, glu=False, dropout=0.0, zero_init=False
    ):
        super().__init__()
...
class CrossAttention(nn.Module):  # not used, never mind
    ...
    def forward(
        ...
        **kwargs,
        ...
        q, k, v = map(
            lambda t: rearrange(t, "b n (h d) -> b h n d", h=num_heads), (q, k, v)
        )
...
class MemoryEfficientCrossAttention(nn.Module):  # we are using this implementation
    ...
    def __init__(
        ...
        angle_control=False,  # Adds rotation + one more dimension
        **kwargs,
    ):
        super().__init__()
        # print(
        ...
        self.angle_dims = (4) * 128 + 24
        if angle_control:
            self.k_adapter_angle_control = nn.Linear(
                self.angle_dims, inner_dim, bias=False
            )
            nn.init.zeros_(self.k_adapter_angle_control.weight)
            self.v_adapter_angle_control = nn.Linear(
                self.angle_dims, inner_dim, bias=False
            )
            nn.init.zeros_(self.v_adapter_angle_control.weight)
    ...
    def forward(
        ...
        context = default(context, x)
        if self.action_control and not self.angle_control:
            context, context_actions = (
                context[:, :, : self.context_dim],
                context[:, :, self.context_dim :],
            )
        if self.angle_control and not self.action_control:
            raise NotImplementedError
        if self.action_control and self.angle_control:
            context, context_actions, context_angle = (
                context[:, :, : self.context_dim],
                context[:, :, self.context_dim : self.context_dim + 128 * 19],
                context[:, :, self.context_dim + 128 * 19 :],
            )
        ...
        if self.action_control:
            context_actions = torch.nan_to_num(context_actions)
            k += self.k_adapter_action_control(context_actions)
            v += self.v_adapter_action_control(context_actions)
        if self.angle_control:
            # remove nans from context_angle
            context_angle = torch.nan_to_num(context_angle)
            k += self.k_adapter_angle_control(context_angle)
            v += self.v_adapter_angle_control(context_angle)
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if high_fidelity_control:
            self.attn21 = attn_cls(
                query_dim=dim,
                context_dim=768,
                heads=n_heads,
                dim_head=d_head,
            ...
            self.attn21.to_q.weight.data = self.attn2.to_q.weight.data
            # self.attn21 = zero_module(self.attn21)
    ...
    def forward(
        ...
        if fd_context is not None:
            kwargs.update({"fd_context": fd_context})
        ...
        if self.use_checkpoint:
            ...
            return checkpoint(
                self._forward, x, context, fd_context, use_reentrant=False
            )
        else:
    ...
    def _forward(
        ...
        if hasattr(self, "attn21"):
            if fd_context is None:
                raise ValueError(
                    "fd_context must be provided for high fidelity control"
                )
            x = (
                self.attn21(
                    self.norm21(x),
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                BasicTransformerBlock(
                    ...
                    action_control=action_control,
                    angle_control=angle_control,
                    high_fidelity_control=high_fidelity_control,
                )
