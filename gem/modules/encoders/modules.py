...
class GeneralConditioner(nn.Module):
    ...
    # print('chanted crossattn from 2 to 1')
    ...
    def forward(
        ...
        for emb_idx, embedder in enumerate(self.embedders):
            ...
            with embedding_context():
                if hasattr(embedder, "input_key") and embedder.input_key is not None:
                    ...
                    if embedder.input_key in batch:
                        if (
                            hasattr(embedder, "dino_channels")
                            and "fd_crossattn" in batch
                        ):
                            emb_out = batch["fd_crossattn"]
                            # print(emb_out.sum())
                        elif (
                            hasattr(embedder, "posenet")
                            and "skeletons_context" in batch
                        ):
                            emb_out = batch["skeletons_context"]
                            # print(emb_out.sum())
                        else:
                            try:
                                ...
                                print(
                                    f"Error in embedder {embedder.__class__.__name__} with input key {embedder.input_key}, and index {emb_idx}"
                                )
                                raise e
            ...
            for emb in emb_out:
                ...
                if (
                    ...
                    # this is necessary if we do use learned mask tokens
                    # if embedder.input_key == "img_seq" and "fd_crossattn" in batch:
                    #     emb = embedder.get_uc(emb)
                    # else:
                    emb = torch.zeros_like(emb)
                if out_key in output:
                    if emb.shape[-1] == 768 and out_key == "vector":
                        output[out_key] += emb
                    # self embedder called DinoEncoder
                    elif out_key in ["fd_crossattn", "rendered_poses"]:
                        output[out_key] = emb
                    else:
...
class ConcatTimestepEmbedderND(AbstractEmbModel):
    ...
    def __init__(
        self, outdim, num_features=None, add_sequence_dim=False, disable_fourier=False
    ):
        super().__init__()
        self.timestep = Timestep(outdim)
        self.outdim = outdim
        self.disable_fourier = disable_fourier
...
class VideoPredictionEmbedderWithEncoder(AbstractEmbModel):
    ...
    def forward(self, vid: torch.Tensor) -> Union[
        ...
        if self.skip_encode:
            ...
            # if self.encoder is not torch.nn.Identity:
            if not isinstance(self.encoder, torch.nn.Identity):
                with torch.autocast("cuda", enabled=not self.disable_encoder_autocast):
                    n_samples = (
                        1  # default(self.en_and_decode_n_samples_a_time, vid.shape[0])
                    )
                ...
                vid = rearrange(
                    vid, "(b t) c h w -> b () (t c) h w", t=self.n_cond_frames
                )
...
class DummyFrozenOpenCLIPImagePredictionEmbedder(AbstractEmbModel):
    def __init__(
        self, open_clip_embedding_config: Dict, n_cond_frames: int, n_copies: int
    ):
        super().__init__()
        ...
        self.n_copies = n_copies
        # self.open_clip = instantiate_from_config(open_clip_embedding_config)
        self.dummy_output = torch.zeros(1, 1024)

    def forward(self, vid):
        return self.dummy_output.unsqueeze(0).repeat(vid.shape[0], 1, 1).to(vid.device)
