...
from gem.modules.diffusionmodules.sampling import (
    EulerEDMSampler,
...
def init_model(version_dict, load_ckpt=True, device="cuda"):
    config = OmegaConf.load(version_dict["config"])
    model = load_model_from_config(
        config, version_dict["ckpt"] if load_ckpt else None, device=device
    )
    return model
...
def initial_model_load(model, device="cuda"):
    global lowvram_mode
    if lowvram_mode:
        model.model.half()
...
def load_model_from_config(config, ckpt=None, device="cuda"):
    ...
    model = initial_model_load(model, device)
    model.eval()
    return model
...
def perform_save_locally(save_path, samples, mode, dataset_name, sample_index):
    assert mode in ["images", "grids", "videos", "videos_bbox", "npy"]
    ...
    if mode == "images":
        ...
        if "real" in save_path:
            ...
            img_seq = 255.0 * img_seq
        npy_save_path = os.path.join(merged_path, f"{dataset_name}_{sample_index:06}.npy")
        np.save(npy_save_path, img_seq.astype(np.uint8))
...
def init_sampling(
    ...
    discretization_config = get_discretization(discretization, sigma_max, rho)
    guider_config = get_guider(guider, cfg_max_scale, cfg_min_scale, num_frames)
    print("guider_config", guider_config)
    print("discretization_config", discretization_config)
    sampler = get_sampler(sampler, steps, discretization_config, guider_config)
    return sampler


def get_discretization(discretization, sigma_max=150, rho=3):
    if discretization == "LegacyDDPMDiscretization":
        ...
        discretization_config = {
            ...
            "params": {"sigma_min": 0.002, "sigma_max": sigma_max, "rho": rho},
        }
    else:
        raise NotImplementedError
...
def get_batch(keys, value_dict, N: Union[List, ListConfig], device="cuda"):
    ...
    for key in keys:
        if key in value_dict:
            ...
            elif key in ["img_seq", "fd_crossattn", "skeletons_context", "rendered_poses"]:
...
def get_condition(model, value_dict, num_samples, force_uc_zero_embeddings, device, chunk_size=25):
    load_model(model.conditioner, device)
    c_total = {}
    uc_total = {}
...
def do_sample(
    ...
    with torch.no_grad(), precision_scope(device), model.ema_scope("Sampling"):
        c, uc = get_condition(
            model, value_dict, num_frames, force_uc_zero_embeddings, device=device
        )

        load_model(model.first_stage_model, device)
        ...
        load_model(model.denoiser, device)
        load_model(model.model, device)
        ...
        noise = torch.randn((num_frames, 8, z.shape[2], z.shape[3])).to(device)
        sample = sampler(
            denoiser,
            noise,
            cond=c,
        ...
        sample[0, :4] = z[0, :4]
        samples_z[:num_frames, :4] = sample[:, :4]
