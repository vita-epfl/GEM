...
def compute_distance_matrix_flow(poses, disps, intrinsics):
    ...
    if not isinstance(poses, SE3):
        poses = torch.from_numpy(poses).float().to(disps.device)[None]
        poses = SE3(poses).inv()

        disps = torch.from_numpy(disps).float().to(disps.device)[None]
        intrinsics = torch.from_numpy(intrinsics).float().to(disps.device)[None]

    N = poses.shape[1]

    ii, jj = torch.meshgrid(torch.arange(N), torch.arange(N), indexing="ij")
    ii = ii.reshape(-1).to(disps.device)
    jj = jj.reshape(-1).to(disps.device)
